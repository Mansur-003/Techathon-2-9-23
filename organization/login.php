...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM `organization` WHERE `name`='$username'";
    ...
    if ($num > 0) {
        ...
        if (password_verify($password, $rows['password'])) {
            session_start();
            $_SESSION['organ_username'] = $username;
            $_SESSION['organ_loggedin'] = true;
            header("location: ../dashboard/organization/index.php");
            exit;
        } else $showerror = true;
    } else $showerror = true;
}
