        ...
        if ($error === 0) {
            if ($img_size > 125000) {
                ...
                if (in_array($img_ex_lc, $allowed_exs)) {
                    ...
                    $img_upload_path = '../dashboard/organization/profile_pictures/' . $new_img_name;
                    move_uploaded_file($tmp_name, $img_upload_path);

                    $result = false;
                    $sql = "SELECT `name` FROM `organization` WHERE `name`='$username'";
                    ...
                    if ($rows == 1) echo "<script>alert('Username Already Exists. Kindly Choose another Username!')</script>";
                        ...
                        if ($password == $cpassword) {
                            ...
                            $sql = "INSERT INTO `organization`(`name`,`email`,`password`,`address`,`about`,`img`) VALUES('$username','$email','$hash','$address','$about','$new_img_name')";
                            $result = mysqli_query($conn, $sql);
...
<body>
            ...
            <div class="register">
                ...
                <form action="" method="post" class="form" enctype="multipart/form-data">
                    ...
                    <div class="form__field">
                        <input required type="text" name="username" maxlength="50" placeholder="Organization Name">
                    </div>
                    <div class="form__field">
                        <input required type="email" maxlength="100" name="email" placeholder="info@mailaddress.com">
                    </div>
                    <div class="form__field">
                        <input required type="text" maxlength="120" name="address" placeholder="Address">
                    </div>
                    <div class="form__field">
                        <textarea required name="about" id="about" cols="30" rows="10" maxlength="1200" placeholder="About Your Organization.." style="border-radius: 20px;"></textarea>
                    </div>
                    <div class="form__field">
    ...
    <script>
        ...
        // $showerror = false;
        // $showalert = false;
        if (<?php echo ($exists ? 'true' : 'false') ?>) {
            alert("Sorry, User Already Exists. Kindly Choose another username!");
        } else if (<?php echo ($passnotmatch ? 'true' : 'false') ?>) {
            alert("Sorry, Passwords do not match!");
        } else if (<?php echo ($showerror ? 'true' : 'false') ?>) {
            alert("Sorry, There is some error. Kindly try again later!");
        } else if (<?php echo ($showalert ? 'true' : 'false') ?>) {
            alert("Congratulations, You have Successfully joined HelpLink!");
        } else if (<?php echo ($em != "" ? true : false) ?>) {
            alert(<?php echo $em ?>)
        }
    </script>
