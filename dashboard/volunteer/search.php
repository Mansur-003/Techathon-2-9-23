...
">
        ...
        <div class="wrapper">
            ...
            <h3 style="text-align: center; color:black;">Search Result for <?php 
            ...
            <div class="search_results">
                <?php
            include 'C:/xampp/htdocs/Hackathon 2/db_con.php';
            if ($sort == 'N') $sql = "SELECT * from `event` where match(`name`,`des`,`location`,`work`,`time`) against ('$search_text') ORDER BY `posted_at` DESC";
            else $sql = "SELECT * from `event` where match(`name`,`des`,`location`,`work`,`time`) against ('$search_text') ORDER BY `posted_at` ASC";
            $result = mysqli_query($conn, $sql);
